      ...
      <div class="input-section">
        ...
        <div v-if="!inputMessage.trim() && messages.length > 1" class="suggestions">
          <button
            ...
            @click="addQuickMessage(suggestion.value)"
          >
            {{ suggestion.label }}
          </button>
        </div>
...
const clearChat = () => {
  if (confirm('确定要清空所有对话吗？')) {
    ...
    // 添加欢迎消息
    addMessage('欢迎使用YooPic 智能体！我可以帮您高效整合多种来源的图片和文字素材，并且以文件的形式美观呈现出来。为您的工作流提高效率😄', false)
  }
}
...
.file-download:hover {
  background: linear-gradient(135deg, #00FFFF, #0b0621);
  transform: scale(1.05);
}
